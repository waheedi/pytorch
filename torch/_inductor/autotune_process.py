...
class GroupedTritonBenchmarkRequest():
    def __init__(self, choices):
        ...
        self.example_bmreq = self.choices[0].bmreq

    def do_benchmark(self, choice_to_callable: Dict[TritonTemplateCaller, Callable], target: float) -> Dict[TritonTemplateCaller, float]:
        timings = {}
        ...
        for choice, _callable in choice_to_callable.items():
            ...
            estimate = sum([start_event.elapsed_time(end_event) for start_event, end_event in event_pairs]) / estimation_iterations
            estimations[choice] = estimate

        target = min(target, estimations[min(estimations, key=estimations.get)])
        ...
        for choice, _callable in choice_to_callable.items():
            ...
            timing = np.quantile([start_event.elapsed_time(end_event) for start_event, end_event in event_pairs], 0.5)
    ...
    def benchmark(
        ...
        *input_tensors: torch.Tensor,
        ...
        if output_tensor is None:
            assert len(input_tensors) == 0
            input_tensors = tuple(x.to_tensor() for x in self.example_bmreq.input_tensor_meta)
            output_tensor = self.example_bmreq.output_tensor_meta.to_tensor()

        choice_to_callable = {}
        for choice in self.choices:
            try:
                _callable = choice.bmreq.make_run_fn(*input_tensors, output_tensor=output_tensor)
